namespace CommitPrompt {
    const inquirer = require('inquirer');
    const chalk = require('chalk');
    const constants = require('./constants');

    const getQuestionData = (commitDescriptionLength: number,
                             components: string[],
                             issueTrackerName: string | undefined,
                             issueTrackerIdType: string) => {
        const questions = [{
            ...
            choices: components,
            validate: function (input: any): boolean | string {
            ...
            type: 'input',
            transformer: (value = '', flags: { isFinal?: boolean }) => {
                const percent = value.length / commitDescriptionLength * 100;
                const color =
                    percent < 80 ? chalk.green : percent > 100
                                                 ? chalk.red
                                                 : chalk.yellow;
    ...
    const askQuestion = (commitDescriptionLength: number,
                         components: string[],
                         issueTrackerName: string,
                         issueTrackerIdType: string) =>
        inquirer.prompt(getQuestionData(commitDescriptionLength, components, issueTrackerName, issueTrackerIdType));
